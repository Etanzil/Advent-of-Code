...
def is_guard_in_a_loop(grid, start_position, start_direction):
    ...
    position2, direction2, exited_grid = run_step(grid, start_position, start_direction)
    if exited_grid:
        return False
    while position1 != position2 or direction1 != direction2:
        position1, direction1, _ = run_step(grid, position1, direction1)
        position2, direction2, exited_grid = run_step(grid, position2, direction2)
        if exited_grid:
            return False
        position2, direction2, exited_grid = run_step(grid, position2, direction2)
        if exited_grid:
...
def part_one(raw_input):
    ...
    visited_positions = get_visited_positions_part_one(grid, start_position, start_direction)
    return len(visited_positions)

def check_if_setting_position_solid_makes_a_loop(grid, position, start_direction, start_position):
    result = False
    add_solid_object_to_grid(grid, position)
    if is_guard_in_a_loop(grid, start_position, start_direction):
        result = True
...
def part_two(raw_input):
    ...
    for position in visited_positions:
        if not position == start_position:
            if check_if_setting_position_solid_makes_a_loop(grid, position, start_direction, start_position):
